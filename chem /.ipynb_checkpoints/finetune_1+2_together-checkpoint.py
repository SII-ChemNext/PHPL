...
import sys
import random
from model import GNN_graphpred,GNN_together
...
def eval(args, model, device, loader_list, epoch): # para_contrastive_loss,
    cl_valid_loader,vdss_valid_loader,t1_2_valid_loader,ic_50_valid_loader= loader_list
    ...
    for step, (cl_batch, vdss_batch, t1_2_batch, ic_50_batch) in enumerate(
            tqdm(zip(cl_valid_loader, vdss_valid_loader, t1_2_valid_loader, ic_50_valid_loader),
...
def main():
    ...
    writer = SummaryWriter(f'runs/0410/finetune/{args.experiment_name}')  
    ...
    num_tasks=1
    #set up model
    model = GNN_together(args.num_layer, args.emb_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type, classnum=args.dataset_type)
...
# 计算每个数据集的样本数
    ...
    for epoch in range(1, args.epochs+1):
        ...
        cl_train_loader = DataLoader(cl_train_sample, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers) #,sampler=cl_train_sampler
        vdss_train_loader = DataLoader(vdss_train_sample, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers) #,sampler=vdss_train_sampler
        t1_2_train_loader = DataLoader(t1_2_train_sample, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers) #,sampler=t1_2_train_sampler
        ic_50_train_loader = DataLoader(ic_50_train_sample, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers) #,sampler=ic_50_train_sampler
        ...
        val_mae_loss = eval(args, model, device, val_loader_list, epoch) # para_contrastive_loss,
        # if dist.get_rank() == 0:
        writer.add_scalar('Loss/train', train_mae_loss, epoch)
        writer.add_scalar('Loss/valid', val_mae_loss, epoch)
        if epoch % 10 == 0 or epoch == 1 or epoch == 2:
            torch.save({'model_finetune': model.state_dict()}, args.save + f"{args.experiment_name}/{epoch}.pth")
        if val_mae_loss < best_val_mae_loss:
            best_val_mae_loss = val_mae_loss
            best_epoch = epoch 
            wait = 0
            torch.save({'model_finetune': model.state_dict()}, args.save + f"{args.experiment_name}/best_model.pth")
        else:
            wait +=1
