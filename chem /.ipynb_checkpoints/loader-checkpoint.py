...
def atom_to_motif(mol):
    ...
    if len(res) != 0:
        for bond in res:   #移除键
            ...
            cliques.append([bond[0][1]])    #移除键后增加原子（单原子也可以是子结构，在后面的步骤中会合并）


    # merge cliques
    for c in range(len(cliques) - 1):
        if c >= len(cliques):   #因为后面删除了，所以需要判断一下
            break
        for k in range(c + 1, len(cliques)):   #循环合并
            if k >= len(cliques):   
                break
            if len(set(cliques[c]) & set(cliques[k])) > 0:    # 判断是否有重合
                cliques[c] = list(set(cliques[c]) | set(cliques[k]))   #合并
                cliques[k] = []    
        cliques = [c for c in cliques if len(c) > 0]   #删除空列表
    cliques = [c for c in cliques if n_atoms>= len(c) > 0]   #删除空列表,需要等号因为如果brics判断不可分，那natoms=len

    # 将环提取出来 
    num_cli = len(cliques)
    ssr_mol = Chem.GetSymmSSSR(mol)   #获取环，需要这一步是因为我想获取原分子的环的原子编号，而不是子图的编号
    for i in range(num_cli):   #考虑环
        c = cliques[i]
        cmol = Chem.MolFromSmiles(get_clique_mol(mol, c))   #获取子图
            ...
            for ring in ssr_mol:
                if len(set(list(ring)) & set(c)) == len(list(ring)):   
                    ...
                    for bond in mol.GetBonds():
                        ...
                        if (start_atom_idx in ring_set and end_atom_idx not in ring_set) or \
                           (end_atom_idx in ring_set and start_atom_idx not in ring_set):
                            connections.append(bond)

                    cliques.append(list(ring))
                    cliques[i] = list(set(cliques[i]) - set(list(ring)))  #删除环

    cliques = [c for c in cliques if n_atoms>= len(c) > 0]   #包含了该mol的所有的子图，形式是该motif包含的原子序号

    #motif list in cliques
    motif = []
    atom_num_list = []
    num_cli = len(cliques)
    for i in range(num_cli):   
        c = cliques[i]
        atom_num = len(c)
        cmol = get_clique_mol(mol, c)   #获取子图，smiles格式
        motif.append(cmol)  #该mol包含的所有子图的smile形式
        atom_num_list.append(atom_num)

    for bond in res:
        atoms, bond_type = bond    
        if not any(set(atoms) in set(c) for c in cliques):
            a1=atoms[0]
            a2=atoms[1]
            bond_con = mol.GetBondBetweenAtoms(a1, a2) 
            connections.append(bond_con)
        ...
        for bond in connections:
            ...
            edge_feature = [allowable_features['possible_bonds'].index(
                bond.GetBondType())] + [allowable_features[
                                            'possible_bond_dirs'].index(
                bond.GetBondDir())]
            for motifs in cliques:
                if i in motifs:
                    m=cliques.index(motifs)+n_atoms
                if j in motifs:
                    n=cliques.index(motifs)+n_atoms
            edges_list.append((m, n))
            edge_features_list.append(edge_feature)
            edges_list.append((n, m))
            edge_features_list.append(edge_feature)#有向边，每个边需要一个edge——future，不然后面再聚合的时候样本数量就不对了
    ...
    data = Data(edge_index=edge_index, edge_attr=edge_attr, motif = motif, atom_num = atom_num_list)
...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
        ...
        elif self.dataset == 'qm9_train' or self.dataset == 'qm9_valid' or self.dataset == 'qm9_test' or self.dataset == 'qm9' or self.dataset == 'qm9_final':
            input_path = self.raw_paths[0]
            ...
            for i in range(len(smiles_list)):
                ...
                if rdkit_mol != None and rdkit_mol.GetNumAtoms() != 1:
                    ...
                    for j,motif in enumerate(data_motif.motif):
                        motif_feature = [self.motif_list.index(motif)+119]+[data_motif.atom_num[j]+4]
        ...
        elif self.dataset == 'auc_train':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            
            # fingerprint_list = pd.read_csv('auc_train_fingerprint_init.csv')
            # descriptor_list = pd.read_csv('auc_train_init_descritors.csv')
            ...
            for i in range(len(smiles_list)):
                ...
                try:
                    ...
                    if rdkit_mol != None:
                        ...
                        # data.fingerprint = torch.Tensor(fingerprint_list[i])
                        # data.descriptor = torch.Tensor(descriptor_list[i])
                        for j,motif in enumerate(data_motif.motif):
                            motif_feature = [self.motif_list.index(motif)+119]+[data_motif.atom_num[j]+4]
                            motif_feature_list.append(motif_feature)
                        data_motif.x = torch.tensor(np.array(motif_feature_list), dtype=torch.long)
        ...
        elif self.dataset == 'auc_valid':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            
            # fingerprint_list = pd.read_csv('auc_valid_fingerprint_init.csv')
            # descriptor_list = pd.read_csv('auc_valid_init_descritors.csv')
        ...
        elif self.dataset == 'lipo_train':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            smiles_list = list(input_df.smiles)
            # label_list = list(input_df.auc_label)
            label_list = list(input_df.value)
            count = 0
            for i in range(len(smiles_list)):
                s = smiles_list[i]
                try:
        ...
        elif self.dataset == 'auc_true_train':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            smiles_list = list(input_df.smiles)
            # label_list = list(input_df.auc_label)
            ...
            for i in range(len(smiles_list)):
                ...
                try:
                    ...
                    continue   
        elif self.dataset == 'auc_true_valid':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            smiles_list = list(input_df.smiles)
            # label_list = list(input_df.auc_label)
        ...
        elif self.dataset == 'lipo_com_train':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            smiles_list = list(input_df.smiles)
            # label_list = list(input_df.auc_label)
            label_list = list(input_df.value)
            cal_label_list = list(input_df.cal_value)
            count = 0
            for i in range(len(smiles_list)):
                s = smiles_list[i]
                try:
                    ...
                    if rdkit_mol != None:
                        ...
                        data.id = torch.tensor([count])
                        data.y = torch.tensor(label_list[i])
                        data.cal_y = torch.tensor(cal_label_list[i])
                        count += 1
                        print(count)
                        data_list.append(data)
                        data_smiles_list.append(smiles_list[i])
                    ...
                    continue    
        elif self.dataset == 'lipo_com_valid':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            smiles_list = list(input_df.smiles)
            # label_list = list(input_df.auc_label)
        ...
        elif self.dataset == 'cl_true_train':
            print('hhhh')
            input_path = self.raw_paths[0]
            print(input_path)
        ...
        elif self.dataset == 'cl_true_valid':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            smiles_list = list(input_df.smiles)
            # label_list = list(input_df.label)
        ...
        elif self.dataset == 't1_2_true_train':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            smiles_list = list(input_df.smiles)
            label_list = list(input_df.value)
        ...
        elif self.dataset == 't1_2_true_valid':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            smiles_list = list(input_df.smiles)
            label_list = list(input_df.value)
        ...
        elif self.dataset == 'vdss_true_train':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            smiles_list = list(input_df.smiles)
            label_list = list(input_df.value) 
        ...
        elif self.dataset == 'vdss_true_valid':
            input_path = self.raw_paths[0]
            input_df = pd.read_csv(input_path)
            smiles_list = list(input_df.smiles) 
            label_list = list(input_df.value) 
