...
def train(args, vdss_model, t1_2_model, cl_model,  para_log_vars, para_function_cl_vdss_t1_2, para_function_auc_cl,  device, loader, optimizer, epoch,temperature=1.0,num_samples = 5,beta=1.0): # para_contrastive_loss,
    vdss_model.train()
    t1_2_model.train()
    cl_model.train()
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        count += 1 
        vdss_mean,vdss_std= vdss_model(batch.x, batch.edge_index, batch.edge_attr, batch.batch, num_samples, False)
        t1_2_mean,t1_2_std= t1_2_model(batch.x, batch.edge_index, batch.edge_attr, batch.batch, num_samples, False)
        cl_mean,cl_std= cl_model(batch.x, batch.edge_index, batch.edge_attr, batch.batch, num_samples, False)
        ...
        # #elbo方法   计算模型kl     
        # kl_loss_cl = cl_model.bayes.kl_loss()
        # kl_loss_vdss = vdss_model.bayes.kl_loss()
        # kl_loss_t1_2 = t1_2_model.bayes.kl_loss()
    
        # cl_loss = (nll_cl+kl_loss_cl*args.temperature).mean()
        # vdss_loss = (nll_vdss+kl_loss_vdss*args.temperature).mean()
        # t1_2_loss = (nll_t1_2+kl_loss_t1_2*args.temperature).mean()        
        ...
        cl_loss = (weights_cl * loss_cl).sum()
        vdss_loss = (weights_vdss * loss_vdss).sum()
        t1_2_loss = (weights_t1_2 * loss_t1_2).sum()

        # elbo_losses = torch.stack([cl_loss, vdss_loss, t1_2_loss])
        # mae_losses  = torch.stack([loss_auc, loss_k3])
        # elbos_norm = (elbo_losses - elbo_losses.mean()) / (elbo_losses.std() + 1e-6)
        # maes_norm = (mae_losses - mae_losses.mean()) / (mae_losses.std() + 1e-6)
        # task_losses = torch.cat([elbos_norm, maes_norm])
        # task_weights = torch.softmax(task_losses / temperature, dim=0) * task_losses.size(0)
        # losses=torch.cat([elbo_losses,mae_losses])
        # loss =torch.sum(task_weights * losses)
        
        # weight = torch.softmax(torch.stack([cl_loss,vdss_loss,t1_2_loss])/ temperature, dim=0)
        # loss =loss_auc + loss_k3 + torch.sum(weight * torch.stack([cl_loss,vdss_loss,t1_2_loss]))
...
def eval(args, vdss_model, t1_2_model, cl_model, para_function_auc_cl,  device, loader, epoch,num_samples=50): # para_contrastive_loss,
    vdss_model.eval()
    t1_2_model.eval()
    cl_model.eval()
    ...
    #mse+高斯
    # loss_cl = F.mse_loss(y_scores_cl, y_cl, reduction='none')
    # loss_vdss = F.mse_loss(y_scores_vdss, y_vdss, reduction='none')
    # loss_t1_2 = F.mse_loss(y_scores_t1_2, y_t1_2, reduction='none')
    
    # nll_cl =nll_gaussian(loss_cl,std_cl)
    # nll_vdss =nll_gaussian(loss_vdss,std_vdss)
    # nll_t1_2 =nll_gaussian(loss_t1_2,std_t1_2) 
...
def objective(trail):
    batchsize = trail.suggest_int('batchsize', 16, 32)
    lr = trail.suggest_float('lr', 0, 1e-4,step=0.0001)
    weightdekey = trail.suggest_float('weight_dekay', 0, 1,step=0.01)
    decay= trail.suggest_float('decay',0,1e-4,step=0.0001)
...
def main():
    ...
    parser.add_argument('--vdss_model_scale', type=float, default=10)
    ...
    para_log_vars = nn.Parameter(torch.zeros(3)) 
    ...
    writer = SummaryWriter(f'runs/0422/final_nll/{args.experiment_name}')  
    ...
    vdss_model = GNN_Bayes(args.num_layer, args.emb_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type, classnum=args.dataset_type)
    t1_2_model = GNN_Bayes(args.num_layer, args.emb_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type, classnum=args.dataset_type)
    cl_model = GNN_Bayes(args.num_layer, args.emb_dim, num_tasks, JK = args.JK, drop_ratio = args.dropout_ratio, graph_pooling = args.graph_pooling, gnn_type = args.gnn_type, classnum=args.dataset_type)
    
    if not args.input_vdss_model_file == "":
        print("load pretrained model from:", args.input_vdss_model_file)
        checkpoint = torch.load(args.input_vdss_model_file, map_location=device)['model_vdss']
        filtered_state_dict = {k: v for k, v in checkpoint.items() if not "graph_pred_linear" in k}
        vdss_model.load_state_dict(filtered_state_dict, strict=False)
    
    if not args.input_t1_2_model_file == "":
        print("load pretrained model from:", args.input_t1_2_model_file)
        checkpoint = torch.load(args.input_t1_2_model_file, map_location=device)['model_t1_2']
        filtered_state_dict = {k: v for k, v in checkpoint.items() if not "graph_pred_linear" in k}
        t1_2_model.load_state_dict(filtered_state_dict, strict=False)
        
    if not args.input_cl_model_file == "":
        print("load pretrained model from:", args.input_cl_model_file)
        checkpoint = torch.load(args.input_cl_model_file, map_location=device)['model_cl']
        filtered_state_dict = {k: v for k, v in checkpoint.items() if not "graph_pred_linear" in k}
        cl_model.load_state_dict(filtered_state_dict, strict=False)
    ...
    model_param_group.append({'params': cl_model.gnn.parameters(), "lr":args.lr * args.vdss_model_scale})
    ...
    model_param_group.append({'params': para_log_vars})
    ...
    model_param_group.append({"params": vdss_model.bayes.parameters(), "lr":args.lr*args.lr_scale})
    model_param_group.append({'params': cl_model.bayes.parameters(), 'lr':args.lr*args.lr_scale })
    model_param_group.append({'params': t1_2_model.bayes.parameters(), "lr":args.lr * args.t1_2_model_scale})
    ...
    for epoch in range(1, args.epochs+1):
        ...
        train_mae_loss = train(args, vdss_model, t1_2_model, cl_model, para_log_vars, para_function_cl_vdss_t1_2, para_function_auc_cl, device, train_loader, optimizer, epoch) # para_contrastive_loss,
        train_mse_loss_list.append(train_mae_loss)
        print(para_function_cl_vdss_t1_2, para_function_auc_cl,)
        auc_val_loss, cl_val_loss, vdss_val_loss, t1_2_val_loss, all_loss, total_nll_cl, total_nll_vdss, total_nll_t1_2 =eval(args, vdss_model, t1_2_model, cl_model, para_function_auc_cl, device, val_loader, epoch) # para_contrastive_loss,
