...
def add_noise_to_partial_labels(y, noise_ratio=0.05, portion=0.2):
    """
    对 y 中的部分值添加相对高斯噪声。

    :param y: 标签张量 shape=(N, 1)
    :param noise_ratio: 噪声占原值的比例
    :param portion: 被扰动的样本比例（如0.2表示20%标签加噪声）
    :return: 添加噪声后的 y
    """
    N = y.size(0)

    # 随机生成掩码：True 的位置将被加噪声
    mask = torch.rand(N, 1, device=y.device) < portion

    # 相对比例噪声，带下限
    noise = torch.randn_like(y) * noise_ratio

    # 应用噪声到被掩码的位置
    y_noisy = y + noise * mask.float()
        ...
        * mask rate number of atom indices
        ...
        if masked_atom_indices == None:
            ...
            if num_atoms != 0:
                ...
                print(data.x)
        # if masked_atom_indices == None:
        #     masked_atom_indices = list(range(num_atoms))
        # create mask node label by copying atom feature of mask atom
        mask_node_labels_list = []
        for atom_idx in masked_atom_indices:
            mask_node_labels_list.append(data.x[atom_idx].view(1, -1))
        if mask_node_labels_list != []:
            data.mask_node_label = torch.cat(mask_node_labels_list, dim=0)
            data.masked_atom_indices = torch.tensor(masked_atom_indices)
        else:
            data.mask_node_label = torch.tensor([[]])  
            data.masked_atom_indices = torch.tensor([]) 
        # print(data.mask_node_label)
        # print(data.masked_atom_indices)
        # ----------- graphMAE -----------
        try:
            atom_type = F.one_hot(data.mask_node_label[:, 0], num_classes=47182).float()
            ...
            N = data.mask_node_label.shape[0] 
            data.node_attr_label = torch.empty((N, 47182))
