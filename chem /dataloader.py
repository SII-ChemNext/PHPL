...
class DataLoaderMaskingPred(torch.utils.data.DataLoader):
    ...
    def __init__(self, dataset, batch_size=1, shuffle=True, mask_rate=0.0, mask_edge=0.0, num_atom_type=119, **kwargs):
        self._transform = MaskAtom(num_atom_type = num_atom_type, num_edge_type = 5, mask_rate = mask_rate, mask_edge=mask_edge)   #增加mask
        super().__init__(
            dataset,
            batch_size,
            shuffle,
    ...
    def collate_fn(self, batches):
        ...
        a1 = BatchMasking.from_data_list(batchs)
        a2 = BatchMasking.from_data_list(batches)
        a1.origin_x = a2.x.clone()  #将未mask处理的数据加入
